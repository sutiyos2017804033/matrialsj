...
class Penjualanadmin extends CI_Controller
{
	...
	function index()
	{
		if ($this->session->userdata('akses') == '1' || $this->session->userdata('akses') == '2') {
			...
			$this->load->view('admin/v_penjualanadmin', $data);
		} else {
	...
	function simpan_penjualanadmin()
	{
		if ($this->session->userdata('akses') == '1' || $this->session->userdata('akses') == '2') {
			...
			if (!empty($total) && !empty($jml_uang)) {
				if ($jml_uang < $total) {
					...
					if ($order_proses) {
						...
						$this->load->view('admin/alert/alert_sukses_admin');
					} else {
						redirect('admin/penjualanadmin');
					}
				}
	...
	function hapus_penjualan()
	{
		if ($this->session->userdata('akses') == '1') {
			...
			$url = base_url('admin/penjualanadmin');
			echo "
			<script>
	...
	function detail_penjualan($jual_nofak)
	{
		if ($this->session->userdata('akses') == '1' || '3') {
			$detail = $this->m_penjualan->detail_penjualan($jual_nofak);
			$data['detail'] = $detail;
			$this->load->view('admin/v_detailpenjualan', $data);
		} else {
			echo "Halaman tidak ditemukan";
		}
	}
